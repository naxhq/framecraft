...
**CLOSED the same day, the larger half: embossed text now gets the gap
treatment engraved text has.** `geom/lettering.py` gains `merge_emboss_gaps`
and `widen_joins`, mirrored by `mergeEmbossGaps` and `widenJoins` in
`lib/engine/solid/lettering.ts`, wired into `build` and `buildLettering`
between the stroke/counter rules and the gap check. The shape is the engraved
merge's: find the sub-nozzle parts of the complement (here the void between
raised letters), bridge them into the material, repeat up to four passes,
then bring the necks that leaves up to the target. A join is a stroke, so it
is widened to the full emboss wall; the gate's own reading of the void is
still taken last, so a gap the merge cannot close still refuses ("...even
after joining the pairs a nozzle cannot part..."). The G6 string that exposed
this, sans `2026-09-06` at the 4.80 mm the band allows, is now BUILT with its
`0`-`6` joined over about 1.2 mm and the validator reads the mesh
`lettering PASS 9 strokes of 1 piece(s); 0.800 mm stroke / 1.022 mm ridge`
(was refused; before the refusal, `FAIL 0.208 mm embossed gap`).

Three decisions, each measured rather than argued:

1. **Which gaps are filled: a DISC opening, not the gate's mitred one.** The
   gate (`thicken.narrowest_width`) opens a void with GEOS's mitred buffer at
   `RESIDUE_MITRE_LIMIT = 10`, and a mitre join at the mouth of a slit spikes
   up to ten radii into it. That is fine for the WIDTH of what survives and
   useless for how far two letters run together: measured on the sans strings
   at the band cap, GEOS reads the 2.47 mm slot between Milano's `i` and `l`
   as 1.52 mm, covers a 1.7 mm slot in Kalamazoo entirely, and leaves a 1.2 mm
   wedge between the date's digits at full length. Worse for the mirror,
   Clipper2 squares a join GEOS truncates, so its mitred residue of the same
   void is a different shape (the fragments recorded above), and no per-mouth
   correction reproduces GEOS: `06` (round, 0.24 mm wide, 1.2 mm) and
   Toronto's `or` (round, 0.29 mm, 1.0 mm) read alike in every summary number
   and GEOS fills one and not the other. A disc of 16 segments is the same
   operation in both libraries. Its residue CONTAINS the mitred one on every
   string measured, so filling it leaves nothing the gate fails, and it reads
   the honest length of a slot. The price: the reference now joins pairs the
   mitre could not see - the mono date's `0`-`9` and `0`-`6` at 0.28-0.30 mm
   over 0.7 mm, which the validator passed at 0.491 on the mesh only because
   the spikes covered them. They are under the project's own 0.36 mm floor
   and print as a clean short bridge at the digits' equators; the pictures were
   looked at.
2. **A slot fuses; a wedge touches.** Filling the gap between two straight
   stems (`Il`, `ll`, `li`) runs the whole height the stems share and prints
   one clean bar where the user typed two letters, which the layout's "will
   touch where they are closest" did not promise. `EMBOSS_JOIN_MAX_EM = 0.4`:
   a filled stretch longer than 0.4 em (area over inscribed width, exact for a
   slot, short for a wedge) refuses the line, naming the gap, the run and the
   size that keeps the pair apart. Derivation: two bowls of a quarter-em
   radius that already touch join over `2 sqrt(2 (em/4) 0.36)`, 1.5-1.9 mm at
   4.5-5 mm; two x-height stems less their rounded ends run 2.2 mm and up.
   Measured at the band cap: the date's `06` 1.19 mm, Brooklyn's `oo` 1.03,
   Kalamazoo's `al` 1.74 (joined); Milano's `il` 2.47, Illinois's `Il`, `ll`,
   `li` 2.48-3.35 (refused). A legibility rule, not a printability one - both
   outcomes print - and stated as such in the code.
3. **The join widening is confined to the joins.** The engraved path re-runs
   the appendage rule over the whole groove after its merge; at the two-nozzle
   emboss target a blanket pass would also move strokes the merge never
   touched. `widen_joins` grows only a thin part that touches a bridge.
   Counters are never filled whole - that is the counter rule's question -
   only their sub-nozzle tails, which rounds an `A`'s apex by under a nozzle.

**Parity.** `fixtures/emboss-gaps-expected.json` (generated by
`test_lettering.py`, asserted by `text.test.ts` through `buildModel`) pins
eleven lines: built or refused, pairs joined, fusion or not. Three faces of
the date (2, 2, 1 joined), Illinois and Milano (fused), mono Illinois and sans
`FRAMECRAFT` at 5 mm (clean), Brooklyn, Toronto, LOOP, Kalamazoo (2, 2, 1, 5).
Both engines decide all eleven alike; the second decimal of a gap is each
library's own (the date's `06` reads 0.21 in GEOS, 0.24 in Clipper2). One
known divergence, PINNED in the same fixture as a `divergences` row rather
than left out (the `matrix.probes.ts:KNOWN_DEFECTS` idiom: the row, the
reason beside it, each engine held to ITS number, and a row whose numbers
become equal is stale and fails): sans `HELLO WORLD` at 8 mm has a stretch AT
the area floor, 0.157 mm^2 in GEOS against 0.194 in Clipper2, so the
reference joins one pair and the engine two. It is a count difference by
construction for a straddle this wide (0.31 mm: 0.5-0.6 mm long, far under
the 0.4 em fusion rule, and under the floor on one side means under the
gate's floor too), and the test asserts built and not-fused on both sides.
Not bounded for every geometry: a floor-area stretch narrower than about
`0.4 / em` mm (0.08 mm at 4.8 mm) would be long enough to trip the fusion
rule on one side only. None has been seen; the row would go red if one did.
`fixtures/lettering-expected.json` regenerates byte-identical: the layout
math did not move.

**Numbers that moved, and where they are written.** `make gate-v2` is
`GATE-V2 PASS`. Its G6 export now logs, for the mono `{date}` on the right
edge, "1 pair(s) of its raised letters came within 0.28 mm of each other ...
and were joined where they touch; 5.55 mm would keep them apart", and the
validator reads `lettering PASS 121 strokes of 7 piece(s); 0.450 mm stroke /
0.491 mm ridge` (was 122 strokes at the same two numbers: the joined pair is
one stroke now, and the gate's mitred reading never saw the 0.28 mm gap, which
is the whole point of decision 1). One pair, not the fixture row's two,
because `bake.py` stamps `{date}` in UTC and that 22:05 CDT run baked
`2026-09-07`; the same line dated `2026-09-06` joins two, so G6's join count
moves with the calendar while its verdict does not (its two stretches read
0.185 and 0.169 mm^2 in GEOS, 0.203 and 0.185 in Clipper2, above the floor
on both sides). The gate is calendar-proof by construction, and only by
construction: `Makefile:513` and its three siblings (516, 521, 524) grep
`lettering +PASS +[1-9][0-9]* strokes of [1-9][0-9]* piece`, a NON-ZERO
stroke count and never a literal one (the comment at `Makefile:441` records
why: `PASS` alone was not enough). Whoever tightens that regex to a literal
stroke count builds a gate that fails on one day in two. The Makefile
comment's figures are left as the history of why the row was re-cut. Nothing in the six preset cities has embossed text
(checked: `presets-index.json` and both matrix `print-params-*.json` carry no
engravings, the v1 golden none); the matrix was re-run anyway and every
verdict is unmoved - reference pipeline six of six `ALL CHECKS PASS`, browser
engine 0.874 / 0.8873 / 1 / 0.8174 / 1.14 and Tokyo's pre-existing `min_wall
FAIL 0.2539`.

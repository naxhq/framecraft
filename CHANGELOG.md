...
- Fixed inside this release, after it was introduced by it: moving the
  base-ridge merge onto the frame-on path added two new `min_wall` failures on
  Tokyo, taking it from two regions at 0.2539 mm to four at 0.204 mm. The
  merge itself was not the culprit (turning it off made Tokyo far worse, seven
  regions at 0.013 mm); its re-cut loop was, subtracting the grown road from
  the parks footprint exactly and leaving a 0.1 mm sliver of park outside the
  bridge that the 0.2 mm seam rim printed as a 0.204 by 0.45 mm fin from
  z 2.4 to 3.0. Green is now re-repaired against the merged road union, as the
  Python reference always did, and Tokyo is back to the two originally
  documented regions.
...
- Tokyo is the one preset the browser engine does not build to the reference
  validator's satisfaction: `min_wall` on two of 468 sampled regions,
  narrowest 0.2539 mm. Every other row passes, `bodies`, `part_meshes` and
  `degenerate_faces` included, and Chicago (0.874), New York (0.8873), Paris
  (1.0), London (0.8174) and San Francisco (1.14) all read ALL CHECKS PASS.
  ...
  repair relies on, plus a 0.4989 mm reading at z = 4.911 mm that is a mitre
  artefact of the validator's own erosion rather than a thin wall. Two repairs
  were built, measured and backed out; both are written up. It has an owner
  and ships as a stated limitation. The regression that briefly took this to
  four regions at 0.204 mm was found and fixed inside this release, see Fixed
  above.
